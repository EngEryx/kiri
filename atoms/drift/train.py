"""Drift atom trainer — trains on task pattern data."""
...
from kiri.atoms.drift.config import DRIFT_SCHEMA, N_EMBD, N_HEAD, N_LAYER, BLOCK_SIZE, LANGUAGE_NAME
...
def run_anomaly_comparison(atom, lang):
    ...
    # Normal: steady work, 1 added 2 completed, low switching
    normal = lang.encode_observation({
        'T': 1, 'C': 2, 'S': 0, 'H': 10, 'W': 1,
    })
    score_n, details_n = sequence_anomaly_score(atom, normal)

    # Anomalous: scope creep — 8 added, 0 completed, 5 switches
    anomalous = lang.encode_observation({
        'T': 8, 'C': 0, 'S': 5, 'H': 15, 'W': 4,
    })
    score_a, details_a = sequence_anomaly_score(atom, anomalous)

    print(f"\nnormal (steady work, Tuesday morning):")
    ...
    print(f"\nanomalous (scope creep: 8 added, 0 done, 5 switches):")
    ...
    if score_a > score_n:
        print(f"\nmodel correctly finds scope creep more surprising ({score_a:.3f} > {score_n:.3f})")
    else:
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Train the Drift atom')
    ...
    parser.add_argument('--name', default='drift', help='model name (default: drift)')
    ...
    lang = StateLanguage(LANGUAGE_NAME, DRIFT_SCHEMA)
